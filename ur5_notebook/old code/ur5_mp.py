...
class ur5_mp:
    def __init__(self):
        ...
        self.cxy_sub = rospy.Subscriber('robot2/cxy', Tracker2, self.tracking_callback, queue_size=1)
        self.cxy_pub = rospy.Publisher('robot2/cxy1', Tracker2, queue_size=1)
        self.phase = 1
        self.object_cnt = 0
        ...
        self.arm = moveit_commander.MoveGroupCommander(group_name, "robot2/robot_description", ns= "robot2")

        # Get the name of the end-effector link
        self.end_effector_link = self.arm.get_end_effector_link()

        # Set the reference frame for pose targets
        reference_frame = "robot2_tf/base_link"
        ...
        self.end_joint_states[2] = 1.49266
    ...
    def tracking_callback(self, msg):
        ...
        if len(self.pointx)>9:
            ...
            print ("TRACK FLAG IS TRUE HERE")
        if self.phase == 2:
            self.track_flag = False
            self.phase = 1
            print ("TRACK FLAG IS FALSE HERE")


        if (self.track_flag and -0.6 < self.waypoints[0].position.x and self.waypoints[0].position.x < 0.6):
            print ("SATISFIES the waypoint Condition")
    ...
    def execute(self):
        if self.track_flag:
            ...
            if self.phase == 1:
                ...
                if 1-fraction < 0.2:
                    rospy.loginfo("Path computed successfully <This is Within the if condition>. Moving the arm.")
                    num_pts = len(plan.joint_trajectory.points)
                    rospy.loginfo("\n# intermediate waypoints <This is Within the if condition> = "+str(num_pts))
                    self.arm.execute(plan)
                    rospy.loginfo("Path execution complete <This is Within the if condition>.")
                    print("if 1-fraction < 0.2:")
                else:
                    rospy.loginfo("Path planning failed <This is Within the if condition>")
                    print("Else of 1-fraction < 0.2")
            ...
            wpose.position.x = -0.1117929
            wpose.position.y = 0.42576795
            wpose.position.z = 0.40043357

            wpose.orientation.x = -0.4938410
            wpose.orientation.y = 0.49088995
            wpose.orientation.z = 0.50915888
            wpose.orientation.w = 0.50587086
            ...
            print ("Track flag is not activated")
            ...
            # If we have a complete plan, execute the trajectory
            if 1-fraction < 0.2:
                rospy.loginfo("Path computed successfully <This is Within the ELSE condition>. Moving the arm.")
                num_pts = len(plan.joint_trajectory.points)
                rospy.loginfo("\n# intermediate waypoints <This is Within the ELSE condition> = "+str(num_pts))
                self.arm.execute(plan)
                rospy.loginfo("Path execution complete <This is Within the ELSE condition>.")
                print("The second fraction statement within else")
            else:
                rospy.loginfo("Path planning failed <This is Within the ELSE condition>")
                print("The second else fraction statement within else")
