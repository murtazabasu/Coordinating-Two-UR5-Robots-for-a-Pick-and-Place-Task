  ...
  int main(int argc, char **argv) {
    ...
    //   gazebo_msgs::ApplyBodyWrench::Response apply_wrench_resp;
     ...
     // ROS_INFO_STREAM("urdf: \n" <<red_xmlStr);
      ...
      spawn_model_req.initial_pose.position.x = -0.125;
      spawn_model_req.initial_pose.position.z = 0.205;
      spawn_model_req.initial_pose.position.y = -0.829176;
      ...
      ros::Duration duration_temp(0, 0);
    ...
    //   apply_wrench_req.reference_frame = "world";
      ...
      while (ros::ok()){
          ...
          //spawn_model_req.initial_pose.position.y = (float)rand()/(float)(RAND_MAX) * -0.3;  // random between -0.4 to 0.4
          std::vector<double> values{-0.829176}; //-0.8391,-0.84,-0.822, ,-0.83, -0.8291, -0.831 
          int i_x = rand() % values.size();
          spawn_model_req.initial_pose.position.y = values[i_x];
